...
// using std::cout;
// using std::endl;
// constexpr size_t Count = 1e9;
// int main() {
//     // std::set<size_t> prime;
//     bitset<Count> *p=new bitset<Count>;
//     p->set();    //每个元素置1
//     for (size_t i = 2; i <= std::sqrt(Count); ++i)
//         if (p->test(i)) {
//             // prime.emplace(i);
//             //第i位为0返回false,为1返回true;
//             for (size_t j = i*i; j < p->size(); j += i)
//                 p->reset(j);    //每个元素置0
//         }
...
bool is_prime(uint64 n) {  // reference: http://miller-rabin.appspot.com
    ...
    static const uint32 bases[][7] = { { 2, 3 },
                                       { 2, 299417 },
                                       { 2, 7, 61 },
