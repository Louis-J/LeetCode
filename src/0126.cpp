...
class Solution0 {
    static inline bool jump(string &s1, string &s2) {
        size_t i;
...
public:
    vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
        if(wordList.size() <= 1) {
            ...
            else if(wordList[0] == endWord && jump(beginWord, endWord))
                return vector<vector<string>>{{beginWord, endWord}};
            else
                return vector<vector<string>>{};
        }
        int W = 0;
        ...
        for(size_t i = 0; i < N; i++) {
            ...
            for(size_t j = i+1; j < N; j++) {
                graph[i*N+j] = graph[j*N+i] = jump(wordList[i], wordList[j]);
            }
        }
        auto pointsFrom = new int[N*N];
        for(size_t i = 0; i < N*N; i++)
            pointsFrom[i] = 0;
        auto pointsHave = new bool[N];
        for(size_t i = 0; i < N; i++)
            pointsHave[i] = false;
        ...
        for(i = 2;; i++) {
            ...
            for(size_t j = 0; j < N; j++) {
                if(!pointsHave[j])
                    for(size_t k = 1; k < term.size(); k++) {
                        if(graph[j*N+term[k]]) {
                            pointsFrom[j*N+term[k]] = i;
                            if(!pointsHave[j]) {
                                pointsHave[j] = true;
                                newterm.emplace_back(j);
                            }
                        }
                    }
            }
        ...
        vector<vector<string>> ret{{beginWord}};
        for(size_t j = 0; j < i; j++) {
            const auto M = road.size();
            for(size_t k = 0; k < M; k++) {
                auto p = road[k][j];
                for(size_t l = 0; l < N; l++)
                    if(pointsFrom[p*N+l] != 0) {
                        if(road[k].size() > j+1) {
                            road.emplace_back(road[k]);
                            road[road.size()-1][j+1] = l;
                            ret.emplace_back(ret[k]);
                            ret[ret.size()-1][j+1] = wordList[l];
                        } else {
                            road[k].emplace_back(l);
                            ret[k].emplace_back(wordList[l]);
                        }
                    }
            }
        }
...
private:
    ...
    unordered_map<string, vector<string>> map;//用unordered_map是因为查找key的时间复杂度O(1),找到了key，对应的value就是树的下一层了。(vector<vector<string>>的查找就太慢了)，用map的key来存储原string,用value来存储原string所有的下一层string(wordSet中所有的只与原string相差一个字符的word)
public:
    vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
        //BFS把找最短序列过程中的每一层的erase后的wordSet中的word们都统计出，包括相遇的那层的word们。然后用dfs去搜索最短序列，因为只统计到相遇那一层，层数就那么多，所以dfs找到的序列层数一定是那么多，所以一定是最短序列。
...
class Solution {
    ...
    static void dfs(vector<string>& wordList, vector<vector<size_t>> &jumpMap, vector<vector<string>> &ret, vector<string>& vec, size_t beginWord, size_t endWord) {
        if (beginWord == endWord) {
            ret.push_back(vec);
            return;
        }
        //对于BFS出的哈希表，单层交给迭代，整棵树交给递归
        for (auto &i : jumpMap[beginWord]) {
            vec.emplace_back(wordList[i]);
            dfs(wordList, jumpMap, ret, vec, i, endWord);
            vec.pop_back();
        }
    }
public:
    vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
        ...
        auto bit = wordMap.find(beginWord);
        size_t B;
        if(bit != wordMap.end())
            B = bit->second;
        else {
            B = wordList.size();
            wordList.emplace_back(beginWord);
            wordMap.emplace(beginWord, B);
        }

        auto eit = wordMap.find(endWord);
        if(eit == wordMap.end())
            return vector<vector<string>>{};
        size_t E = eit->second;

        if(jump(beginWord, endWord))
            return vector<vector<string>>{{beginWord, endWord}};

        vector<vector<size_t>> jumpMap(wordList.size());

        unordered_map<string, size_t> beginSet {{beginWord, B}};
        unordered_map<string, size_t> endSet {{endWord, E}};
        for(bool direct = false, connect = false;;) {
            unordered_map<string, size_t> next;
            for(auto &w : beginSet)
                wordMap.erase(w.first);
                        ...
                        *(char*)&(word[i]) = j;
                        ...
                        if(it != wordMap.end()) {
                            if (!endSet.count(word))
                                next.emplace(word, it->second);
                            else
                                connect = true;  //双端set搜索相遇了，在添加完这一整层的word后退出循环。

                            if(direct)
                                jumpMap[it->second].emplace_back(posi);
                            else
                                jumpMap[posi].emplace_back(it->second);
                        }
                    }
                    *(char*)&(word[i]) = oric;
                    // word[i] = oric;
        ...
        vector<string> vec {beginWord};
        dfs (wordList, jumpMap, ret, vec, B, E);
        return ret;
    }
};
...
int main() {
    cout << "1:" << endl;
    {
        ...
        cout << Solution().findLadders("hit", "cog", wordList) << endl;
    }
    cout << "2:" << endl;
    {
    ...
    cout << "3:" << endl;
    {
        vector<string> wordList{"dot", "dog", "lot", "log", "cog"};
        cout << Solution().findLadders("hit", "cog", wordList) << endl;
    }
    cout << "4:" << endl;
    {
        vector<string> wordList{"hot", "dog", "dot"};
        cout << Solution().findLadders("hot", "dog", wordList) << endl;
    }
    cout << "5:" << endl;
    {
        ...
        cout << Solution().findLadders("nanny", "aloud", wordList) << endl;
    }
    return 0;
}
#endif