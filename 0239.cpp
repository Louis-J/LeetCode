...
class Solution {
    class CirQue0 {
        ...
        int *const que;
    public:
        CirQue0(size_t n): capacity(n), que(new int[n]) {}
        ~CirQue0() {delete[] que;}
        bool empty() {return num == 0;}
        int &front() {return que[start];}
        int &back() {return que[(start+num-1)%capacity];}
        void pop_front() {
    ...
    class CirQue1 {
        int *const que, *const nail;
        int *b, *e;
        bool isempty = true;
    public:
        CirQue1(size_t n): que(new int[n]), nail(que+n-1), b(que), e(que) {}
        ~CirQue1() {delete[] que;}
